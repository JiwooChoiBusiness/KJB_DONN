...
| 5 LLM | guardrails · gemini 체인 · 추출 · 설명 · 제도 KB | 완료 | 체인 12초 상한, 모델 쿨다운, 발화 근거 없는 숫자 폐기. 제도 KB 15편 키워드 검색으로 faq 응답. 설명(f)은 템플릿 우선 |
| 5 LLM | 자유 질의 오케스트레이터 | 예정 (P4) | 도구 호출 |
| 6 API | FastAPI 19 엔드포인트 | 완료 | /api/meta 추가. 테스트 134개 통과 |
| 7 화면 | 홈 · 내 부채 · 공시 비교 · 계정 선택 · 결정 기록 · 소비 패턴 · 생애 흐름 | 완료 | Opus 4회 다듬기: 로고·Pretendard·계정 선택·대화 로그·제도 안내 패널·소비 패턴·생애 흐름 |
| 7+ 생애주기 층 | 재무비율 · 생애 단계 · 노후 산식 · 목표 시나리오 | 완료 (P7) | 골든 벡터 12개, /api/lifecycle, 연금 KB 3편 |
| 8 평가 | 페르소나 러너 · 리포트 · 데모 스크립트 | 진행 중 (P6) | 골든 질문 YAML, 재현성·금지어·안전모드 검사 |
