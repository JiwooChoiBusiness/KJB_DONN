...
실행 데모(Render): https://kjb-donn.onrender.com (무료 플랜이라 15분 무접속 시 절전, 첫 접속 30초~1분)

생성형 LLM 기반 부채 전문 AI Agent(한국어) PoC입니다. 화면 부제는 "부채 걱정을 덜어드리는 금융 AI Agent"입니다. 순위와 금액 같은 수치는 전부
...
- 마지막 업데이트(2026-09-06): 대화창 파일 첨부(거래내역 → 소비 패턴 요약), 계산형 질의(추가 상환·일시 상환·대환·은퇴 시점), 공시 비교의 이전 결과 대비 변화 표시, 지출 절감을 상환 효과로 연결하는 카드
